...
/// @title V20 - Quadratic adverse + vol-adaptive pHat alpha + higher symmetric (30)
/// @notice Uses devBps^2 scaling for adverse to penalize large deviations more.
///         Uses vol-adaptive alpha for pHat (higher alpha in high vol).
contract Strategy is AMMStrategyBase {
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        // Direction-confirming + vol-adaptive pHat alpha
        ...
        if (gamma != 0) {
            ...
            if (dt > 0) {
                // Base alpha 25%, adjust by vol: higher vol → faster tracking
                uint256 volBpsNow = wadToBps(emaAbsMove);
                uint256 baseAlpha = confirming ? 28e16 : 22e16;
                // Boost alpha by up to 8% when vol is high (volBps > 10)
                if (volBpsNow > 10) {
                    uint256 boost = (volBpsNow - 10) * 1e16;  // 1% per bps above 10
                    if (boost > 8e16) boost = 8e16;
                    baseAlpha += boost;
                    if (baseAlpha > 50e16) baseAlpha = 50e16;  // Cap at 50%
                }
                pHat = wmul(pHat, WAD - baseAlpha) + wmul(pImplied, baseAlpha);
                phase = 0;
            } else {
                if (phase < 10) phase += 1;
        ...
        // Quadratic adverse: adds devBps^2/50 for super-linear scaling at large deviations
        uint256 adverseBps = (devBps * 7) / 10 + (volBps * 12) / 10 + (devBps * devBps) / 50;
    ...
    function getName() external pure override returns (string memory) { return "OptV20"; }
}
