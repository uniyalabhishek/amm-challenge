...
/// @title V10 - Baseline + same-step 2% + smarter symmetric zone
/// @notice Small refinements: same-step EMA 2%, vol-scaled symmetric fee,
///         new-step boost +5, all else same as baseline.
contract Strategy is AMMStrategyBase {
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        if (devBps < 8) {
            // Vol-scaled symmetric fee
            uint256 symBps = 28;
            if (volBps >= 20) symBps = 29;
    ...
    function getName() external pure override returns (string memory) { return "OptV10"; }
}
