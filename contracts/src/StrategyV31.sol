...
/// @title V31 - High-confidence-only asymmetry
/// @notice Only use asymmetric fees when devBps is HIGH (>=20), indicating strong trend.
///         At devBps < 20: symmetric fee at 28 bps.
///         At devBps >= 20: asymmetric with higher favorable floor (10 bps).
///         This limits asymmetry to cases where direction accuracy is likely >70%.
contract Strategy is AMMStrategyBase {
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        // Standard pHat update (no direction-adaptive, simpler)
        ...
        // HIGH threshold for asymmetry: only when very confident (devBps >= 20)
        if (devBps < 20) {
            // Wide symmetric zone
            bidFee = bpsToWad(28);
            askFee = bpsToWad(28);
        } else {
            // Asymmetric with high adverse, moderate favorable floor
            uint256 adverseBps = (devBps * 7) / 10 + (volBps * 12) / 10;
            if (dt > 0) adverseBps += 4;
            if (adverseBps < 38) adverseBps = 38;

            // Higher favorable floor: 10 bps (safe because high confidence in direction)
            uint256 favorBps;
            if (devBps >= 50) { favorBps = 10; }
            else { favorBps = 20 - (devBps - 20) * (20 - 10) / (50 - 20); }
    ...
    function getName() external pure override returns (string memory) { return "HighConf31"; }
}
