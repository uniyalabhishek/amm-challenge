...
/// @title V6d - Baseline + only change: pHat EMA 30% instead of 25% on new step
contract Strategy is AMMStrategyBase {
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        if (gamma != 0) {
            ...
            // CHANGE: 30% EMA instead of 25%
            if (dt > 0) { pHat = wmul(pHat, WAD - 30e16) + wmul(pImplied, 30e16); phase = 0; }
    ...
    function getName() external pure override returns (string memory) { return "V6d"; }
}
