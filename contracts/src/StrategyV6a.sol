...
/// @title V6a - Baseline + only change: raise favorable floor from 1 to 5 bps
contract Strategy is AMMStrategyBase {
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        // CHANGE: raise favorable floor from 1 to 5
        uint256 favorBps;
        if (devBps <= 8) { favorBps = 20; }
        else if (devBps >= 50) { favorBps = 5; }  // was 1
        else { favorBps = 20 - (devBps - 8) * (20 - 5) / (50 - 8); }  // was 20-1
    ...
    function getName() external pure override returns (string memory) { return "V6a"; }
}
