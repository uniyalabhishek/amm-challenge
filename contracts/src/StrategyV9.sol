...
/// @title V9 - High adverse, keep low favorable (test LVR theory)
/// @notice Hypothesis: arb losses scale with σ² regardless of fee (LVR).
///         So higher adverse fee is strictly better. But favorable MUST stay low
///         to keep pHat estimation accurate (gamma correction effect).
contract Strategy is AMMStrategyBase {
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        if (gamma != 0) {
            ...
            else { if (phase < 10) phase += 1; pHat = wmul(pHat, WAD - 1e16) + wmul(pImplied, 1e16); }
        ...
        // CHANGE: Much higher adverse fees
        uint256 adverseBps = (devBps * 8) / 10 + (volBps * 15) / 10;
        if (dt > 0) adverseBps += 6;
        if (adverseBps < 50) adverseBps = 50;  // Much higher floor

        // Keep favorable very low (same as baseline)
        uint256 favorBps;
    ...
    function getName() external pure override returns (string memory) { return "OptV9"; }
}
