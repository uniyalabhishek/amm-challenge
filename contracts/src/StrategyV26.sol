...
/// @title V26 - Use raw spot for pHat (no gamma correction) + direction-adaptive
/// @notice Simplest approach: just use spot directly for pHat, no gamma correction.
///         After arb, spot ≈ fairPrice / gamma ≈ fairPrice * (1 + fee).
///         The systematic bias is small (~25 bps) and constant for all sides.
///         This completely eliminates direction-dependent bias.
///         Trade-off: small systematic bias vs zero direction-dependent noise.
contract Strategy is AMMStrategyBase {
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        // CORE CHANGE: Use raw spot for pHat, no gamma correction at all
        // After arb: spot = fairPrice / gamma ≈ fairPrice * (1 + fee)
        // The bias is ~25 bps regardless of direction, so direction-neutral
        if (dt > 0) {
            uint256 alpha = confirming ? 28e16 : 22e16;
            pHat = wmul(pHat, WAD - alpha) + wmul(spot, alpha);
            phase = 0;
        } else {
            if (phase < 10) phase += 1;
            pHat = wmul(pHat, WAD - 1e16) + wmul(spot, 1e16);
        }
    ...
    function getName() external pure override returns (string memory) { return "OptV26"; }
}
