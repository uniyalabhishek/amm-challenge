...
/// @title V8 - "Smart Spread" approach
/// @notice Instead of directional asymmetry for retail, focus on total spread.
///   Key insight: retail edge ≈ (bidFee + askFee) * volume.
///   Volume barely changes with fee diffs. So maximize total spread while
///   keeping the adverse side high to protect from arb.
///   Approach: set BOTH fees moderately high, with adverse slightly higher.
contract Strategy is AMMStrategyBase {
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        // Adverse fee: same formula as baseline but with higher floor
        uint256 adverseBps = (devBps * 7) / 10 + (volBps * 12) / 10;
        if (dt > 0) adverseBps += 4;
        if (adverseBps < 38) adverseBps = 38;

        // "Smart" favorable: set as % of adverse to maintain high total spread
        // favorBps = max(adverseBps * 0.4, floor)
        uint256 favorBps;
        if (devBps <= 8) {
            favorBps = 20;
        } else {
            favorBps = (adverseBps * 4) / 10;  // 40% of adverse
            if (favorBps < 10) favorBps = 10;
            if (favorBps > 28) favorBps = 28;
        }
    ...
    function getName() external pure override returns (string memory) { return "OptV8"; }
}
