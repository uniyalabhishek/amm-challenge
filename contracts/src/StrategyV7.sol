...
/// @title V7 - Combine best ablation results + vol-adaptive symmetric fee
/// @notice Changes from baseline:
///   - Same-step EMA 2% (V6e showed best)
///   - Vol-adaptive symmetric fee (higher vol → higher fee)
///   - Adverse floor 40 bps (slight increase)
///   - Slightly higher adverse new-step boost (+5 instead of +4)
contract Strategy is AMMStrategyBase {
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        if (gamma != 0) {
            ...
            if (dt > 0) {
                ...
                if (phase < 10) phase += 1;
                // Same-step EMA 2% (was 1% in baseline)
        ...
        if (dt > 0) adverseBps += 5;  // slightly higher new-step boost
        if (adverseBps < 40) adverseBps = 40;  // slightly higher floor

        // --- Favorable fee (keep same as baseline) ---
        uint256 favorBps;
        ...
        // --- Assignment with vol-adaptive symmetric fee ---
        if (devBps < 8) {
            // Vol-adaptive: higher vol → slightly higher symmetric fee
            uint256 symBps = 27;
            if (volBps > 15) {
                symBps = 28;
            }
    ...
    function getName() external pure override returns (string memory) { return "OptV7"; }
}
