...
/// @title V24 - Average-gamma pHat correction + direction-adaptive alpha
/// @notice KEY INSIGHT: Using actual gamma (from whichever fee was applied) for pImplied
///         introduces direction-dependent bias. When direction is wrong, pImplied barely
///         corrects (low favorable fee → gamma close to 1). When correct, it over-corrects
///         (high adverse fee → gamma far from 1).
///         FIX: Use the AVERAGE of bid and ask fees for gamma correction. This makes
///         pHat estimation independent of which side the trade was on, reducing
///         the direction-dependent bias that accumulates over time.
contract Strategy is AMMStrategyBase {
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        // CORE CHANGE: Use average gamma instead of side-specific gamma
        // This eliminates direction-dependent bias in pHat estimation
        uint256 gammaAvg = WAD - (lastBid + lastAsk) / 2;
        if (gammaAvg != 0) {
            // For buy (AMM buys X): price was higher, so spot is above fair → multiply down
            // For sell (AMM sells X): price was lower, so spot is below fair → multiply up
            uint256 pImplied = trade.isBuy ? wmul(spot, gammaAvg) : wdiv(spot, gammaAvg);
            if (dt > 0) {
    ...
    function getName() external pure override returns (string memory) { return "OptV24"; }
}
