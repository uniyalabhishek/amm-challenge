...
/// @title V27 - Fixed gamma (25bps) for pHat + higher favorable floor (8bps) + dir-adaptive
/// @notice Combines two key insights:
///   1. Fixed gamma for pHat decouples estimation from fee structure → better direction accuracy
///   2. Higher favorable floor (8 bps) reduces arb loss when direction is wrong
///      WITHOUT harming pHat accuracy (since pHat doesn't use actual fee)
///   The gamma decoupling allows higher fees without pHat distortion penalty.
contract Strategy is AMMStrategyBase {
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        // Fixed gamma at 25 bps for pHat (decoupled from actual fees)
        uint256 gammaFixed = WAD - bpsToWad(25);
        {
            uint256 pImplied = trade.isBuy ? wmul(spot, gammaFixed) : wdiv(spot, gammaFixed);
            if (dt > 0) {
        ...
        // Higher favorable floor: 8 bps (safe because pHat is decoupled)
        uint256 favorBps;
    ...
    function getName() external pure override returns (string memory) { return "OptV27"; }
}
