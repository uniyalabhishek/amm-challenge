...
/// @title V28 - Average gamma pHat + favorable floor 5 + symmetric 30 + dir-adaptive
/// @notice Combines average-gamma pHat correction with moderate fee improvements.
///         Average gamma = midpoint of bid+ask, removes direction bias.
///         Higher favorable floor (5 bps) for better arb protection.
///         Higher symmetric fee (30 bps) for more retail edge in neutral zone.
contract Strategy is AMMStrategyBase {
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        // Average gamma: midpoint of bid and ask fees
        ...
        // Favorable floor 5 bps
        uint256 favorBps;
        if (devBps <= 8) { favorBps = 20; }
        else if (devBps >= 50) { favorBps = 5; }
        else { favorBps = 20 - (devBps - 8) * (20 - 5) / (50 - 8); }
    ...
    function getName() external pure override returns (string memory) { return "OptV28"; }
}
