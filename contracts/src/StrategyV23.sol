...
/// @title V23 - Confidence-weighted asymmetry + vol-scaled symmetric
/// @notice Key theory: asymmetric fees only help if direction accuracy > 70%.
///         For small devBps (low confidence), use mostly symmetric.
///         For large devBps (high confidence), use strongly asymmetric.
///         Smooth transition: favorable fee scales with direction confidence.
///         Also: vol-scaled symmetric fee (higher in high vol to protect against arb).
contract Strategy is AMMStrategyBase {
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        // Adverse fee (same formula)
        uint256 adverseBps = (devBps * 7) / 10 + (volBps * 12) / 10;
        if (dt > 0) adverseBps += 4;
        if (adverseBps < 38) adverseBps = 38;

        // Confidence-weighted favorable: smooth transition from symmetric to asymmetric
        // At devBps=8: favorable = 20 bps (almost symmetric with adverse ~38)
        // At devBps=20: favorable = 12 bps (moderate asymmetry, ~65% confidence)
        // At devBps=50: favorable = 3 bps (strong asymmetry, ~90% confidence)
        uint256 favorBps;
        ...
        // Vol-scaled symmetric fee: higher when vol is high to protect against arb
        // Base 28 + volBps * 0.3, capped at 35
        uint256 symBps = 28;
        if (volBps > 8) {
            symBps = 28 + (volBps - 8) * 3 / 10;
            if (symBps > 35) symBps = 35;
        }

        if (devBps < 8) {
            bidFee = bpsToWad(symBps);
            askFee = bpsToWad(symBps);
        } else if (spot > pHat) {
            askFee = favor;
            bidFee = adverse;
        } else {
    ...
    function getName() external pure override returns (string memory) { return "OptV23"; }
}
