#!/usr/bin/env python3
"""Test parameter interactions missed by coordinate descent."""
...
contract Strategy is AMMStrategyBase {{
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {{
        ...
        if (gamma != 0) {{
            ...
            if (dt > 0) {{
                uint256 alpha = confirming ? {ac}e16 : {at}e16;
                pHat = wmul(pHat, WAD - alpha) + wmul(pImplied, alpha);
                phase = 0;
            }} else {{
                if (phase < 10) phase += 1;
                pHat = wmul(pHat, WAD - {as_}e16) + wmul(pImplied, {as_}e16);
            }}
        ...
        if (dt > 0) adverseBps += {bo};
        if (adverseBps < {af}) adverseBps = {af};
        uint256 favorBps;
        if (devBps <= {th}) {{ favorBps = 20; }}
        else if (devBps >= 50) {{ favorBps = 1; }}
        else {{ favorBps = 20 - (devBps - {th}) * 19 / (50 - {th}); }}
...
SOL = "/home/user/amm-challenge/contracts/src/Sweep.sol"
...
def run(params):
    ...
    try:
        out = subprocess.run(
            ["amm-match", "run", SOL, "--simulations", "20"],
...
# Current best from sweep
BEST = {"ac": 30, "at": 22, "as_": 2, "sf": 28, "af": 40, "bo": 3, "th": 8}

# Test interactions
configs = [
    ("Best (sweep)", BEST),
    ("ac32/at20", {**BEST, "ac": 32, "at": 20}),
    ("ac32/at18", {**BEST, "ac": 32, "at": 18}),
    ("ac30/at20", {**BEST, "ac": 30, "at": 20}),
    ("af42/bo2", {**BEST, "af": 42, "bo": 2}),
    ("af42/bo3", {**BEST, "af": 42, "bo": 3}),
    ("af40/bo2", {**BEST, "af": 40, "bo": 2}),
    ("sf29/af40", {**BEST, "sf": 29, "af": 40}),
    ("sf27/af40", {**BEST, "sf": 27, "af": 40}),
    ("ac32/at20/af42/bo2", {**BEST, "ac": 32, "at": 20, "af": 42, "bo": 2}),
    ("ac30/at22/af42/bo2/th9", {**BEST, "af": 42, "bo": 2, "th": 9}),
    ("ac28/at22/as3/af42/bo2", {**BEST, "ac": 28, "as_": 3, "af": 42, "bo": 2}),
]
...
for name, params in configs:
    ...
    if edge: print(f"{name}: {edge:.2f}")
    else: print(f"{name}: FAIL")

print(f"\n{'='*50}")
results.sort(key=lambda x: x[1] if x[1] else 0, reverse=True)
for name, edge in results:
    if edge: print(f"{name:<35} {edge:.2f}")
